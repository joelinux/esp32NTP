...
bool debugIsOn = false;   // set to true to see progress results in the serial
                          // monitor, set to false to suppress these
...
void startNTPServer() { Udp.begin(NTP_PORT); }

// ****************************************************************************
//
// ****************************************************************************
void setDateAndTimeFromGPS(void* parameter) {
    if (debugIsOn) Serial.println("Start setDateAndTimeFromGPS task");
    ...
    while (true) {
        ...
        if (gps.date.isValid() &&
            gps.time.isValid())  // make sure the date and time are valid (in
                                 // that values are populated)
        {
            struct tm wt;
            ...
            if ((wt.tm_year > 1900) && (wt.tm_mon > 0) && (wt.tm_mon < 13) &&
                (wt.tm_mday > 0) && (wt.tm_mday < 32) && (wt.tm_hour < 24) &&
                (wt.tm_min < 60) &&
                (wt.tm_sec < 61))  // make sure the values are within reason
            {
                // wait for the PPS flag to be raised (signifying the true start
                // of the candidate time)
                if (xSemaphoreTake(mutex, portMAX_DELAY) == pdTRUE) {
                    // set the date and time

                    // unsigned long pegProcessingAdjustmentEndTime =
                    // rtc.getMicros();
                    int64_t pegProcessingAdjustmentEndTime =
                        esp_timer_get_time();
                    if (debugIsOn)
                        Serial.println(
                            "Candidate date and time " + String(wt.tm_year) +
                            " " + String(wt.tm_mon) + " " + String(wt.tm_mday) +
                            " " + String(wt.tm_hour) + " " + String(wt.tm_min) +
                            " " + String(wt.tm_sec));

                    // set the real time clock
                    rtc.setTime(wt.tm_sec, wt.tm_min, wt.tm_hour, wt.tm_mday,
                                wt.tm_mon, wt.tm_year);
                    stratum = 2;
                    timeandDateSet = true;
                    timeisPPS = true;
...
void startGPSUpdateTask() {
    ...
    xTaskCreatePinnedToCore(
        setDateAndTimeFromGPS, "Set Date and Time from GPS", 3000, NULL,
        20,  // task priority must be reasonably high or the queues from which
             // the gps data is drawn will not be adequately replenished
        &taskHandle0,
...
//
// ****************************************************************************
void startUDPSever() { Udp.begin(NTP_PORT); }
...
uint64_t getCurrentTimeInNTP64BitFormat() {
    ...
    if (cur_ms >= ppsLastSignalmicro) {
        ...
        microseconds =
            uint64_t(cur_ms) - uint64_t(ppsLastSignalmicro) + 4294967296ULL;
    }

    uint64_t ntpts = ((uint64_t)(clockSecondsSinceEpoch + epoch + 1) << 32) |
                     (uint64_t)(microseconds_to_ntp_fraction(microseconds));
    /*
...
void sendNTPpacket(IPAddress remoteIP, int remotePort) {
    ...
    Udp.endPacket();
    syslog.debug("NTP %s", remoteIP.toString().c_str());
}
...
void computeHmac(const char* message, char* hmacResult) {
    ...
    mbedtls_md_hmac_starts(&ctx, (const unsigned char*)admin_pw.c_str(),
                           keyLen);
...
void processNTPRequests() {
    ...
    if (packetSize == NTP_PACKET_SIZE)  // an NTP request has arrived
    {
        ...
        if (timeandDateSet) {
            ...
            // when ok to proceed place a hold on using the mutex to prevent the
            // date and time from being refreshed while the reply packet is
            // being built sendNTPpacket(remoteIP, Udp.remotePort());
            if (xSemaphoreTake(mutex, portMAX_DELAY) == pdTRUE) {
                // send NTP reply
        ...
        // note: unlike other serial monitor writes in this sketch, this
        // particular write is on the critical path for processing NTP requests.
        // while it does not delay the response to an initial NTP request, if
        // subsequent NTP requests are queued up to run directly afterward this
        // serial monitor write will delay responding to the queued request by
        // approximately 1 milli second.
        if (debugIsOn) {
            String updatemessage = "Query from " + remoteIP.toString() +
                                   " at  " + String(rtc.getEpoch());
        ...
        if (packetSize > 0) {
            ...
            if (packet.startsWith("stats")) {
                ...
                sprintf(buffer,
                        ...
                        ESP32NTP_VER, NTP_BOARD_VERSION, tt->tm_hour,
                        tt->tm_min, tt->tm_sec, tt->tm_year + 1900,
                        tt->tm_mon + 1, tt->tm_mday,
                        ppsLock == true ? "YES" : "NO", stratum,
                        gps.satellites.value(), WiFi.SSID(),
                        WiFi.localIP().toString().c_str(), act_cnt,
                        running_act_cnt, WiFi.RSSI(), ESP.getFreeHeap(),
                        ESP.getChipModel(), ESP.getFlashChipSize());
                // sprintf(tmps, "  act: %-4d\n", act_cnt);
            ...
            if (packet.startsWith("display")) {
                toggleDisplay();
                sprintf(buffer, "Display toggled %s",
            ...
            if (debugIsOn)
                Serial.println("Invalid request received on port " +
                               String(NTP_PORT) +
                               ", length =" + String(packetSize));
        }
    }
}
